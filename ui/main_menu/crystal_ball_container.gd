...
func _hover_on():
	if tween_return and tween_return.is_running():
		return
	tween_ball = get_tree().create_tween().bind_node(self).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT).set_parallel(true)
...
func _hover_off():
	if tween_ball and tween_ball.is_running():
		return
	tween_return = get_tree().create_tween().bind_node(self).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT).set_parallel(true)
